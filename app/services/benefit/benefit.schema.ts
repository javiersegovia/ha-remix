...
.object({
   ...
   */
  imageKey: zfd.text(z.string().trim()),
...
.superRefine((data, ctx) => {
  if (data) {
    const { image, imageKey, title, description, buttonText, buttonHref } =
      data

    const values = [title, description, buttonText, buttonHref]
    ...
    if (values.some(Boolean) !== values.every(Boolean)) {
      if (!title) {
        ctx.addIssue({
          code: 'custom',
          path: ['title'],
          message: 'Por favor, agregue un título',
        })
      }
      ...
      if (!buttonText) {
        ctx.addIssue({
          code: 'custom',
          path: ['buttonText'],
          message: 'Por favor, agregue un texto para el botón',
        })
      }
      ...
      if (!buttonHref) {
        ctx.addIssue({
          code: 'custom',
          path: ['buttonHref'],
          message: 'Por favor, agregue un enlace para el botón',
        })
      }
    }
